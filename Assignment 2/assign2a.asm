...
// Version a: Use 32-bit registers for variables delcared using int. Use m4 macros to name the registers to make your code more readable. 
//            Optimize the code so it uses as few instructions as possible. Using gdb, display contents of key registers as the program
//            executes; should show the algorithm is working as expected (script.txt). Also print out the original and reversed values both 
//            in hexidecimal and in binary, just before the program exits.
...
main:       stp x29, x30, [sp, -16]!                                                    // Store X29 and X30 at the address pointed to by SP. [SP, -16]! decreases SP by 16 *BEFORE* this operation is done.
            ...
            mov x_r, 0x07FC07FC                                                         // Initialize x with 0x07FC07FC
...
print:      adrp x0, values                                                             // setting first argm of printf
            ...
            ret                                                                         // "Returns" from this function. Sets the program counter to the value stored in a register. If no register is given, defaults to x30 (AKA LR)
